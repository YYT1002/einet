...
def find_resolution_mapping(G_micro, dist, depth=4,
    ...
    eps_range = (max_res - min_res)*scale
    resolution_ei = []
    resolution_range = np.linspace(min_res, max_res, 3)
    resolution_mappings = []
    ...
    for res in resolution_range:
        ...
        macro_mapping_e = {i: i if lab == -1 else (len(labs_res)+lab)
                           for i, lab in enumerate(labs_res)}
        macro_types_e = {i: 'spatem1' for i in macro_mapping_e.values()
                         if i > max(macro_mapping_e.keys())}
        ...
        EI_macro_e = effective_information(G_macro_e)
        resolution_ei.append(EI_macro_e)
        resolution_mappings.append(macro_mapping_e)
    ...
    if depth == 0:
        ...
        if resolution_ei[1] >= resolution_ei[2] and resolution_ei[1] >= resolution_ei[0]:
            new_max = (resolution_range[1] + resolution_range[2]) / 2 + eps_range
            new_min = (resolution_range[1] + resolution_range[0]) / 2 - eps_range

        elif resolution_ei[0] >= resolution_ei[2] and resolution_ei[0] >= resolution_ei[1]:
            new_max = resolution_range[1] + eps_range
            new_min = resolution_range[0] - eps_range

        else:
            new_max = resolution_range[2] + eps_range
            new_min = resolution_range[1] - eps_range

        return find_resolution_mapping( G_micro, dist, depth=depth-1, min_res=new_min,
                                    max_res=new_max, scale=scale)

import networkx as nx

def causal_emergence_louvain(G, check_inacc=False, t=500):
    r"""
    Given a microscale network, G, this function computes a macroscale mapping,
    macro_mapping, using the Leiden clustering method such that when G is
    recast as a coarse-grained representation, G_macro, the resulting graph's
    $EI$ is higher than the original network.
